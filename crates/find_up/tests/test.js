...
relative.barDirQux = path.join(
  relative.fixtureDirectory,
  name.fooDirectory,
  name.barDirectory,
  name.qux,
);
...
absolute.fixtureDirectory = path.join(absolute.packageDirectory, name.fixtureDirectory);
...
if (!isWindows) {
  test('async (symbolic links)', async (t) => {
    ...
    t.is(
      await findUp(name.directoryLink, { cwd, type: 'directory', allowSymlinks: false }),
      undefined,
    );
  });
  ...
  test('sync (symbolic links)', (t) => {
    ...
    t.is(
      findUpSync(name.directoryLink, { cwd, type: 'directory', allowSymlinks: false }),
      undefined,
    );
  });
}
...
test('async (matcher function)', async (t) => {
  ...
  t.is(
    await findUp((directory) => directory !== cwd && directory, { type: 'directory' }),
    path.join(cwd, '..'),
  );

  t.is(
    await findUp((directory) => directory === cwd && 'package.json', {
...
test('sync (matcher function)', (t) => {
  ...
  t.is(
    findUpSync((directory) => directory !== cwd && directory, { type: 'directory' }),
    path.join(cwd, '..'),
  );

  t.is(
    findUpSync((directory) => directory === cwd && 'package.json', {
      cwd: absolute.fixtureDirectory,
    }),
    absolute.packageJson,
  );
